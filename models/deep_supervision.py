"""Deep supervision (DS-Head): auxiliary 1x1 heads on intermediate decoder features.

Training loss per auxiliary head is identical to the main head criterion (not Dice-only).
During training, engine.py passes config.criterion into compute_loss; implementations are
in utils.py:
  - binary: BceDiceLoss (BCEWithLogitsLoss + soft Dice)
  - multi-class: CeDiceLoss (CrossEntropyLoss + Dice)
Related: configs/config_setting.py, engine.py (train_one_epoch), utils.py.
"""
import torch
...
class DeepSupervision (nn .Module ):
    ...
    def compute_loss (self ,predictions ,target ,criterion =None ,epoch =None ,total_epochs =None ):
        """Aggregate weighted auxiliary losses.

        Args:
            criterion: Same loss module as the main head (BceDiceLoss or CeDiceLoss from utils.py).
        """
        if criterion is None :
            # Fallback when compute_loss is called without criterion; normal training does not use this path.
            from utils import BceDiceLoss ,CeDiceLoss
            criterion =BceDiceLoss (wb =1 ,wd =1 )if self .num_classes ==1 else CeDiceLoss (self .num_classes )
        ...
        for i ,(pred ,weight )in enumerate (zip (predictions ,self .weights )):
            if self .num_classes ==1 :
                ...
                if pred .shape [-2 :]!=target_for_loss .shape [-2 :]:
                    pred =F .interpolate (pred ,size =target_for_loss .shape [-2 :],mode ='bilinear',align_corners =False )
            else :
                ...
                if pred .shape [1 ]!=self .num_classes :
                    raise ValueError (f'Multi-class deep supervision: logits C={pred .shape [1 ]} vs num_classes={self .num_classes }')
            # Applies BceDiceLoss or CeDiceLoss (utils.py); not a standalone Dice loss.
